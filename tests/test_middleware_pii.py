...
@pytest.mark.asyncio
async def test_scrub_pii_no_entities(mock_analyzer: Any) -> None:
    """Test scrubbing text with no PII."""
    text = "Hello world, this is a safe message."
    mock_analyzer.analyze.return_value = []

    result = await scrub_pii_payload(text)
    assert result == text


@pytest.mark.skipif(not HAS_PRESIDIO, reason="presidio-analyzer not installed")
@pytest.mark.asyncio
async def test_scrub_pii_entities_replacement(mock_analyzer: Any) -> None:
...
@pytest.mark.skipif(not HAS_PRESIDIO, reason="presidio-analyzer not installed")
@pytest.mark.asyncio
async def test_scrub_pii_empty_input(mock_analyzer: Any) -> None:
    """Test empty input returns empty."""
    assert await scrub_pii_payload("") == ""
    assert await scrub_pii_payload(None) is None


@pytest.mark.skipif(not HAS_PRESIDIO, reason="presidio-analyzer not installed")
@pytest.mark.asyncio
async def test_scrub_pii_exception(mock_analyzer: Any) -> None:
    """Test error handling."""
    mock_analyzer.analyze.side_effect = Exception("Analyzer crashed")

    with pytest.raises(ValueError, match="PII Scrubbing failed"):
        await scrub_pii_payload("some text")


@pytest.mark.skipif(not HAS_PRESIDIO, reason="presidio-analyzer not installed")
@pytest.mark.asyncio
async def test_scrub_pii_generic_value_error(mock_analyzer: Any) -> None:
