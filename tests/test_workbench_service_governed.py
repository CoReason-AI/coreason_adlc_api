...
@pytest.fixture
def mock_validation_logic():
    # Mock budget and pii checks
    with patch("coreason_adlc_api.middleware.budget.check_budget_status", return_value=True) as mock_budget, \
...
@pytest.mark.asyncio
async def test_create_draft_governed(workbench_service, mock_service):
    user_oid = uuid.uuid4()
    draft_input = DraftCreate(auc_id="test-auc", title="Test Draft", oas_content={"key": "val"})
    mock_response = DraftResponse(
        draft_id=uuid.uuid4(), user_uuid=user_oid, auc_id="test-auc", title="Test Draft", oas_content={"key": "val"},
        status=ApprovalStatus.DRAFT, mode=AccessMode.EDIT, created_at=datetime.now(timezone.utc), updated_at=datetime.now(timezone.utc)
    )
...
@pytest.mark.asyncio
async def test_publish_artifact_governed(workbench_service, mock_service):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
    signature = "valid-sig"
    ...
    result = await workbench_service.publish_artifact(draft_id=draft_id, request=request, user_oid=user_oid, signature=signature)
    assert result == {"url": mock_url}

@pytest.mark.asyncio
async def test_list_drafts(workbench_service, mock_service):
...
@pytest.mark.asyncio
async def test_get_draft(workbench_service, mock_service, mock_db_pool):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
    mock_db_pool.fetch.return_value = [{"role_name": "DEVELOPER"}]
    mock_response = DraftResponse(
        draft_id=draft_id, user_uuid=user_oid, auc_id="test-auc", title="Test Draft", oas_content={},
        status=ApprovalStatus.DRAFT, mode=AccessMode.EDIT, created_at=datetime.now(timezone.utc), updated_at=datetime.now(timezone.utc)
    )
...
@pytest.mark.asyncio
async def test_update_draft(workbench_service, mock_service):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_validate_draft(workbench_service, mock_validation_logic):
...
@pytest.mark.asyncio
async def test_validate_draft_pii_failure(workbench_service, mock_validation_logic):
...
@pytest.mark.asyncio
async def test_transition_status(workbench_service, mock_service, mock_db_pool):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
    # Mock MANAGER role
...
@pytest.mark.asyncio
async def test_transition_status_forbidden(workbench_service, mock_service, mock_db_pool):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_assemble_artifact(workbench_service, mock_service):
    draft_id = uuid.uuid4()
    user_oid = uuid.uuid4()
    # Must use kwargs
