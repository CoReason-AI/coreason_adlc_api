...
from coreason_adlc_api.workbench.schemas import DraftCreate, ValidationResponse
...
def test_check_budget_status_under_limit():
    user_id = uuid4()
...
def test_check_budget_status_over_limit():
    user_id = uuid4()
    with patch("coreason_adlc_api.middleware.budget.get_redis_client") as mock_get_client:
        mock_redis = MagicMock()
        mock_get_client.return_value = mock_redis
...
def test_check_budget_status_no_key():
    user_id = uuid4()
    with patch("coreason_adlc_api.middleware.budget.get_redis_client") as mock_get_client:
        mock_redis = MagicMock()
        mock_get_client.return_value = mock_redis
...
def test_check_budget_status_error():
    user_id = uuid4()
    with patch("coreason_adlc_api.middleware.budget.get_redis_client") as mock_get_client:
...
def test_client_validate_draft():
