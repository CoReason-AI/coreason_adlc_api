...
from coreason_adlc_api.workbench.schemas import AgentArtifact, DraftResponse, ApprovalStatus
...
def test_router_assemble_success(client: TestClient, mock_oidc_factory: object, mock_workbench_service: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    mock_assemble, _, mock_get_draft = mock_workbench_service
    draft_id = uuid4()
    # Cast factory to correct type if needed, or rely on runtime behavior. Mypy fixture typing is hard, assume Any for factory.
    ...
    mock_get_draft.return_value = DraftResponse(
        draft_id=draft_id, user_uuid=uuid4(), auc_id="test-project", title="T", oas_content={}, created_at=datetime(2023, 1, 1), updated_at=datetime(2023, 1, 1), status=ApprovalStatus.APPROVED
    )
...
def test_router_assemble_error(client: TestClient, mock_oidc_factory: object, mock_workbench_service: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    mock_assemble, _, mock_get_draft = mock_workbench_service
    draft_id = uuid4()
    token = mock_oidc_factory({"groups": ["manager"]}) # type: ignore
...
def test_router_publish_success(client: TestClient, mock_oidc_factory: object, mock_workbench_service: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
...
def test_router_publish_error(client: TestClient, mock_oidc_factory: object, mock_workbench_service: Tuple[AsyncMock, AsyncMock, AsyncMock]) -> None:
    _, mock_publish, mock_get_draft = mock_workbench_service
    draft_id = uuid4()
    token = mock_oidc_factory({"groups": ["manager"]}) # type: ignore
