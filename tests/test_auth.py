...
@pytest.fixture
def mock_oidc_setup(rsa_key_pair):
...
@pytest.fixture
def mock_token(mock_oidc_setup) -> tuple[str, str, str]:
...
@pytest.mark.asyncio
async def test_parse_token_expired(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
    payload = {
        "oid": str(uuid.uuid4()),
        "exp": datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=1),
...
@pytest.mark.asyncio
async def test_parse_token_invalid_issuer(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
    payload = {
        "oid": str(uuid.uuid4()),
        "iss": "wrong-issuer",
...
@pytest.mark.asyncio
async def test_parse_token_missing_oid_sub(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
    payload = {
        "email": "test@coreason.ai",
        "iss": f"{settings.OIDC_DOMAIN.rstrip('/')}/",
...
@pytest.mark.asyncio
async def test_parse_token_non_uuid_oid(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
    payload = {
        "sub": "auth0|12345",
        "email": "test@coreason.ai",
...
@pytest.mark.asyncio
async def test_parse_token_bad_group_uuid(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
    payload = {
        "sub": str(uuid.uuid4()),
        "email": "test@coreason.ai",
...
@pytest.mark.asyncio
async def test_parse_token_jwks_missing(mock_oidc_setup) -> None:
...
@pytest.mark.asyncio
async def test_parse_token_invalid_signature_error(mock_oidc_setup) -> None:
    private_key = mock_oidc_setup
...
@pytest.mark.asyncio
async def test_auth_endpoints_proxy(mock_oidc_setup) -> None:
    ...
    with patch("coreason_adlc_api.routers.auth.upsert_user", new=AsyncMock()) as mock_upsert:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            ...
            # 1. Device Code
            mock_device_resp = Response(200, json={
                "device_code": "dc-123",
                "user_code": "ABCD-1234",
            ...
            mock_pending = Response(400, json={"error": "authorization_pending"}, request=Request("POST", "https://mock.idp/token"))
            ...
            mock_success = Response(200, json={
...
@pytest.mark.asyncio
async def test_auth_endpoints_upsert_failure(mock_oidc_setup) -> None:
...
async def test_device_code_http_error() -> None:
    ...
    with patch("coreason_adlc_api.routers.auth.get_oidc_config", return_value={"device_authorization_endpoint": "http://url"}):
         with patch("coreason_adlc_api.routers.auth.get_http_client", return_value=mock_client):
             async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                 resp = await ac.post("/api/v1/auth/device-code")
...
async def test_token_poll_public_client() -> None:
     ...
     with patch("coreason_adlc_api.config.settings.OIDC_CLIENT_SECRET", None):
         with patch("coreason_adlc_api.routers.auth.get_oidc_config", return_value={"token_endpoint": "http://url"}):
             ...
             mock_client.__aenter__.return_value = mock_client
             mock_client.__aexit__.return_value = None
             mock_client.post.return_value = Response(200, json={"access_token": "token", "expires_in": 60}, request=Request("POST", "url"))
...
@pytest.mark.asyncio
async def test_token_poll_non_UUID_oid_in_router(mock_oidc_setup) -> None:
...
async def test_token_poll_http_status_error() -> None:
     with patch("coreason_adlc_api.routers.auth.get_oidc_config", return_value={"token_endpoint": "http://url"}):
         ...
         mock_client.__aenter__.return_value = mock_client
         mock_client.__aexit__.return_value = None
         mock_client.post.side_effect = httpx.HTTPStatusError("Gone", request=MagicMock(), response=Response(410, text="Gone"))
