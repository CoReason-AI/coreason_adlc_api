...
@pytest.mark.asyncio
async def test_derive_roles(service, mock_db_pool):
...
@pytest.mark.asyncio
async def test_verify_project_access_success(service):
    with patch(
...
@pytest.mark.asyncio
async def test_verify_project_access_fail(service):
    with patch(
        "coreason_adlc_api.workbench.service_governed.map_groups_to_projects",
...
async def test_create_draft(service):
    ...
    with (
        ...
        patch(
            "coreason_adlc_api.workbench.service.create_draft", new=AsyncMock(return_value=mock_resp)
        ) as mock_create,
    ):
...
@pytest.mark.asyncio
async def test_publish_artifact_strict(service):
    draft_id = uuid4()
    user_oid = uuid4()
    groups = [uuid4()]
    signature = "valid_sig"
    ...
    with (
        patch("coreason_veritas.gatekeeper.SignatureValidator.verify_asset") as mock_verify,
        patch.object(service, "_get_draft_and_verify_access", new=AsyncMock()) as mock_access,
        patch(
            "coreason_adlc_api.workbench.service.publish_artifact", new=AsyncMock(return_value="http://url")
        ) as mock_publish,
    ):
        resp = await service.publish_artifact(
            draft_id=draft_id, signature=signature, user_oid=user_oid, groups=groups
        )
...
@pytest.mark.asyncio
async def test_approve_draft_manager(service):
    draft_id = uuid4()
    user_oid = uuid4()
    groups = [uuid4()]
    ...
    with (
        ...
        patch(
            "coreason_adlc_api.workbench.service.transition_draft_status", new=AsyncMock()
        ) as mock_transition,
    ):
...
@pytest.mark.asyncio
async def test_approve_draft_not_manager(service):
    draft_id = uuid4()
    user_oid = uuid4()
    groups = [uuid4()]
